...
describe('Char8', () => {
  it('Test Char8.computeSBYear', () => {
    expect(Char8.computeSBYear(U.parseDate('2022-02-04'), 2).toString()).toBe('壬寅')
    expect(Char8.computeSBYear(U.parseDate('2022-02-03'), 2).toString()).toBe('辛丑')
    expect(Char8.computeSBYear(U.parseDate('2022-02-03'), -1).toString()).toBe('壬寅')
    expect(Char8.computeSBYear(U.parseDate('2021-10-03'), 2).toString()).toBe('辛丑')
    expect(Char8.computeSBYear(U.parseDate('2016-4-3'), 2).toString()).toBe('丙申')
    expect(Char8.computeSBYear(U.parseDate('1904-10-4'), 2).toString()).toBe('甲辰')
    expect(Char8.computeSBYear(U.parseDate('2020-01-25'), 2).toString()).toBe('己亥')
    expect(Char8.computeSBYear(U.parseDate('2020-02-03 23:00:01'), 2).toString()).toBe('庚子')
    expect(Char8.computeSBYear(U.parseDate('2020-02-03 22:59:59'), 2).toString()).toBe('己亥')
    expect(Char8.computeSBYear(U.parseDate('2020-01-25')).toString()).toBe('庚子')
  })
